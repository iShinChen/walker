...
//			hidden : true,
			handler : function() {
				com.walker.file.info.recommend();
			}
		},'->',{
			xtype : 'button',
...
com.walker.file.info.getGridPnl = function(){
	...
	var reader = new Ext.data.JsonReader({
		fields : ["id","title","summary","content","type","create_time","update_time","status","recommend"],
		root : 'rows',
		id : 'ID',
		totalProperty : 'total'
	});
	var store = new Ext.data.Store({
		url : '/jkcms/info/getInfoListPage',
		reader : reader,
		baseParams : {
			limit : 15,
			start: 0
	...
	var model = new Ext.grid.ColumnModel([sm,
              new Ext.grid.RowNumberer({
				...
				renderer : function(value,metadata){
					return com.walker.common.renderer.getComboValue(com.walker.file.info.info_type, value, '');
				}
			}, {
				header : "概述",
				dataIndex : "summary",
				...
				renderer : function(value) {
					if (value == 1) {
						return '<font color="green">主页推荐</font>';
					}else{
						return '<font color="black">未推荐</font>';
					}
				}
			}
	]);
...
com.walker.file.info.recommend = function(types) {
	var rec = com.walker.common.getSelectRecord(com.walker.file.info.infoGridPnl,true);
	if(rec){
		var sumBtn = function(btn){
			if(btn != 'yes'){
		...
		Ext.Msg.confirm('系统提示',"确定将该资讯设置为主页推荐吗?",sumBtn);
...
com.walker.file.info.info_type = new Ext.data.Store({
	url : '/jkcms/user/getPubCode?PUB_CODE_NAME=INFO_TYPE',
