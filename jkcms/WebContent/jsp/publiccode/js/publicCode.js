...
com.walker.publicCode.getGridPnl = function(){
	...
	var store = new Ext.data.Store({
		url : '/jkcms/user/getPublicCodeListPage',
		reader : reader,
		baseParams : {
			limit : 15,
			start: 0,
...
com.walker.publicCode.update=function(){
	...
	if(rec){
		...
		Ext.Ajax.request({
			url: '/jkcms/user/selectPublicCodeInfoById',
			params : {
				ID:rec.get('ID')
			},
	...
	if(com.walker.publicCode.formPanel.getForm().isValid()){
		...
		var sumBtn = function(btn){
			...
			com.walker.publicCode.formPanel.getForm().submit({
				method:"POST",
				waitMsg:"保存中,请稍后...",
				url : "/jkcms/user/savePublicCodeInfo",		
...
com.walker.publicCode.deletePublicCode=function(){
	...
	if(selReds){
		...
		var sumBtn = function(btn){
			if(btn!='yes'){return;}
			Ext.Ajax.request({
				url: '/jkcms/user/deletePublicCodeInfo',
				params: {
...
com.walker.publicCode.flushBookConfig=function(){
	var sumBtn = function(btn){
		if(btn!='yes'){return;}
		Ext.Ajax.request({
			url: '/jkcms/user/flushBookConfig',
			success: function(response) {
				var result = Ext.decode(response.responseText);
				if(result.success){
